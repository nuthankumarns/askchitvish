...
class getRecipe extends REST_Controller
{
	var $dataDB;
	function recipedetails_get()
    {
...
//var_dump($recipeid);
        $this->load->model('recipedetails');
        $data= $this->recipedetails->get_recipe_details($recipeid);
 $i=0;
        foreach($data as $result)
        {
			$this->dataDB['Result']['Data'][$i]['recipeid']=$result->id!=''?$result->id:"No data";
        	$this->dataDB['Result']['Data'][$i]['recipe']=$result->imgtitle!=''?$result->imgtitle:"No data";
