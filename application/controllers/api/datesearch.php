...
class dateSearch extends REST_Controller
{
	var $dataDB;
	function search_get()
    {
        ...
        $this->load->model('datediff');
        $data= $this->datediff->difference();
        $i=0;
        foreach($data as $result)
        {

        	$this->dataDB['Result']['Data'][$i]['recipe_id']=$result->id;
          	$this->dataDB['Result']['Data'][$i]['recipe']=$result->imgtitle;
				$this->dataDB['Result']['Data'][$i]['category_name']=$result->title;
        	$this->dataDB['Result']['Data'][$i]['upload_date']=$result->upload_date;
			$this->dataDB['Result']['Data'][$i]['category_id']=$result->catid;
